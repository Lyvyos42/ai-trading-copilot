...
import { Clock, Globe, Shield, Zap, Activity, ChevronDown } from "lucide-react";
...
export function CockpitHeader({
  ...
  return (
    <header className="border-b border-border/40 bg-surface-1/90 backdrop-blur-md px-3 py-2 select-none">
      <div className="flex flex-wrap items-center justify-between gap-3">
        {/* Left segment: Platform Identity & Live Clock */}
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 pr-3 border-r border-border/40">
            ...
            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-border/50 bg-surface-2 text-muted-foreground">
              COMMAND HUD
            </span>
          </div>
                ...
                <div
                  ...
                  className={cn(
                    ...
                    isOpen
                      ...
                      : "bg-surface-2/60 text-muted-foreground border-border/30"
                  )}
                >
                  <span
                    className={cn(
                      "h-1.5 w-1.5 rounded-full",
                      isOpen ? "bg-bull animate-pulse" : "bg-muted-foreground/40"
                    )}
                  />
                  <span>{s.code}</span>
        ...
        <div className="flex items-center gap-2">
          {/* Regime Badge */}
          <div className="flex items-center gap-1.5 px-2 py-1 rounded border border-border/40 bg-surface-2 text-[10px] font-mono">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 rounded border border-primary/30 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-mono font-bold transition-colors"
            >
            ...
            {profileDropdownOpen && (
              <div className="absolute right-0 mt-1 w-56 rounded border border-border/60 bg-surface-2 p-1 shadow-2xl z-50 animate-fade-in">
                <div className="text-[10px] font-mono uppercase text-muted-foreground px-2 py-1 border-b border-border/30">
                ...
                {PROFILES.map((p) => (
                  <button
                    ...
                    className={cn(
                      "w-full text-left px-2 py-1.5 rounded text-xs font-mono flex items-center justify-between hover:bg-surface-3 transition-colors",
                  ...
                  >
                    ...
                    <span className="text-[10px] px-1 py-0.5 rounded border border-border/40 bg-surface-1 text-muted-foreground">
                      {p.tf}
                    </span>
                  </button>
                ))}
